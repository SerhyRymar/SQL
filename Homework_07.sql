...
select 
    ...
    tc.CPM,
    tc.ROMI,
      case when tc.CTR > 0 then ROUND (coalesce((tc.CTR - ma.prev_CTR) / ma.prev_CTR * 100 , 0), 3) else 0 end as CTR_change,
      case when ma.prev_CPM > 0 then ROUND (coalesce((tc.CPM - ma.prev_CPM) / ma.prev_CPM * 100 , 0), 3) else 0 end as CPM_change,
      case when ma.prev_ROMI > 0 then ROUND (coalesce((tc.ROMI - ma.prev_ROMI) / ma.prev_ROMI * 100 , 0), 3) else 0 end as ROMI_change
from total_cte tc
